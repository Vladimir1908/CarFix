...
function ServicesTab() {
  const [services, setServices] = useState([
    { icon: "D", name: "Diagnosticare Computerizata", price: "200 MDL", duration: "1h", active: true },
    { icon: "E", name: "Reparatie Motoare Electrice", price: "400 MDL", duration: "2-4h", active: true },
    { icon: "G", name: "Reparatie Generatoare", price: "400 MDL", duration: "2-3h", active: true },
    { icon: "M", name: "Reparatie Mecanica Generala", price: "300 MDL", duration: "1-3h", active: true },
    { icon: "F", name: "Sistem de Franare", price: "250 MDL", duration: "1-2h", active: true },
    { icon: "U", name: "Schimb Ulei & Filtre", price: "150 MDL", duration: "30min", active: true },
  ]);

  const toggle = (i: number) => {
    setServices(prev => prev.map((s, idx) => idx === i ? { ...s, active: !s.active } : s));
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 20 }}>
        {services.map((s, i) => (
          <div key={i} style={{ background: "#fff", borderRadius: 16, padding: "20px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", display: "flex", alignItems: "center", gap: 16, opacity: s.active ? 1 : 0.5, transition: "0.3s" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: "0.75rem", marginTop: 4, color: s.active ? "#10b981" : "#ef4444", fontWeight: "bold" }}>
                {s.active ? "Activ" : "Inactiv"}
              </div>
            </div>
            <div
              onClick={() => toggle(i)}
              style={{ width: 36, height: 20, borderRadius: 10, background: s.active ? "#10b981" : "#ddd", position: "relative", cursor: "pointer", transition: "0.2s" }}
            >
...
export default function DashboardAdmin({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    if (raw) {
      ...
      const demo: Booking[] = [
        { id: 1, service: "Diagnosticare Computerizata", date: "2025-07-15", time: "10:00", name: "Ion Popescu", phone: "+373 69 111 222", email: "ion@mail.md", carModel: "BMW E46", licensePlate: "CBA 001", notes: "", status: "confirmed", createdAt: new Date().toISOString() },
        { id: 2, service: "Reparatie Motoare Electrice", date: "2025-07-16", time: "14:00", name: "Maria Ionescu", phone: "+373 69 333 444", email: "", carModel: "Toyota Corolla", licensePlate: "XYZ 999", notes: "Motor zgomot ciudat la pornire", status: "pending", createdAt: new Date().toISOString() },
        ...
        { id: 4, service: "Sistem de Franare", date: "2025-07-17", time: "11:30", name: "Elena Chiriac", phone: "+373 69 777 888", email: "elena@gmail.com", carModel: "Dacia Logan", licensePlate: "MMM 777", notes: "Frane care scartaie", status: "pending", createdAt: new Date().toISOString() },
        { id: 5, service: "Reparatie Generatoare", date: "2025-07-13", time: "15:00", name: "Victor Moraru", phone: "+373 69 999 000", email: "", carModel: "Ford Focus", licensePlate: "VVV 321", notes: "", status: "cancelled", createdAt: new Date().toISOString() },
      ];
      setBookings(demo);
    }
  }, []);
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #eee", padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            <button onClick={() => setTab("overview")} style={NAV_STYLE(tab === "overview")}>Prezentare generala</button>
            <button onClick={() => setTab("bookings")} style={NAV_STYLE(tab === "bookings")}>
              Programari {stats.pending > 0 && <span style={{ background: "#ff4d29", color: "#fff", borderRadius: 10, padding: "2px 7px", fontSize: "0.75rem", marginLeft: 6 }}>{stats.pending}</span>}
            </button>
        ...
        {tab === "overview" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 20, marginBottom: 40 }}>
              {[
                { label: "Total programari", value: stats.total, color: "#6366f1" },
                { label: "In asteptare", value: stats.pending, color: "#f59e0b" },
                { label: "Confirmate", value: stats.confirmed, color: "#3b82f6" },
                { label: "Finalizate", value: stats.completed, color: "#10b981" },
              ].map(card => (
        ...
        {tab === "bookings" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
              <h2 style={{ margin: 0, color: "#222" }}>Toate programarile</h2>
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                <input
                  value={search}
                  onChange={e => setSearch(e.target.value)}
                  placeholder="Cauta dupa nume, masina, nr..."
            ...
            <div style={{ background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              {filtered.length === 0 ? (
                <div style={{ padding: "60px", textAlign: "center", color: "#aaa" }}>Nu exista programari pentru filtrele selectate.</div>
              ) : (
                filtered.map((b, i) => (
                  <div key={b.id} style={{ padding: "20px 24px", borderBottom: i < filtered.length - 1 ? "1px solid #f3f3f3" : "none", display: "flex", gap: 24, flexWrap: "wrap", alignItems: "flex-start" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-end" }}>
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        {b.status === "pending" && (
                          <button onClick={() => updateStatus(b.id, "confirmed")} style={{ background: "#3b82f6", color: "#fff", border: "none", padding: "5px 12px", borderRadius: 8, cursor: "pointer", fontSize: "0.8rem", fontWeight: "bold" }}>Confirma</button>
                        )}
                        {(b.status === "pending" || b.status === "confirmed") && (
                          <button onClick={() => updateStatus(b.id, "completed")} style={{ background: "#10b981", color: "#fff", border: "none", padding: "5px 12px", borderRadius: 8, cursor: "pointer", fontSize: "0.8rem", fontWeight: "bold" }}>Finalizeaza</button>
                        )}
                        {b.status !== "cancelled" && b.status !== "completed" && (
                          <button onClick={() => updateStatus(b.id, "cancelled")} style={{ background: "#ef4444", color: "#fff", border: "none", padding: "5px 12px", borderRadius: 8, cursor: "pointer", fontSize: "0.8rem", fontWeight: "bold" }}>Anuleaza</button>
                        )}
                      </div>
                    </div>
                    {b.notes && (
